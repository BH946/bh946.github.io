...
* 계산 과정에서 `double` 타입으로 해줘야 오차가 거희 없다.
...
* **또한, "거리가 일정" 할때는 "BFS" 로 "최단거리" 를 구할 수 있는데 이때 DP의 개념이 포함된다.**
  * BFS 특성상 "큐" 를 통해서 **레벨(높이)순**으로 접근하기 때문에 최적의 거리(**dist 배열**)를 항상 기록하므로 DP처럼 최적의 값으로 배열을 채워나간다는 점이 동일하다.
  * DP배열이 1차원, 2차원인지 연습하기 좋은 문제를 소개한다.
    * [숨바꼭질4_BOJ13913](https://www.acmicpc.net/problem/13913), [이모티콘_BOJ14226](https://www.acmicpc.net/problem/14226)
  * 참고로 **dist배열의** 경우 **거리들을 전부 기록해둘 필요가 있을때** 사용하기 좋다.
* **단, "거리일정"과 "최단거리"에서 조건들이 있을때 "돌아서 다른경로로"가는게 더 빠른 문제들도 있다.**
  * "경로의 특징을 저장해둬야 하는 문제" 이런건 DFS가 더 좋다.
    * 예로 outArr[depth] 에 기록했던것 처럼..
  * 다만, 여기선 **"최단거리"** 이기 때문에 BFS로 풀만한 문제들만 나올것이다.
  * **결론은 이때 큐 사이즈만큼 반복해 구분지어 레벨(높이)를 구하자. -> 레벨=최단거리**
    * 코드는 아래쪽에서 확인!!
      * **참고로 바로 아래코드인 `resultTemp++;` 은 너비 구함 -> 높이(레벨) 아님!**
      * **또한, p.dist 처럼 "기록"해서 최소비용 갱신하는 로직도 확인(아래에 코드)**
      
    * 단, "**조건에 걸려 대기**"하는 곳에선 큐에 **다시 삽입**해야한다. **(중요)**
    * 어차피 "**거리는 일정**" 하기(ex:1초씩) 때문이다.
    * 당연한 얘기지만 **visit**도 해야 "최단거리"가 도출 될 것이다.(내생각)
  * 예시 문제 : SWEA : 4193. 수영대회 결승전
  * 참고 : [나와 같은 고찰을 느낀 다른분](https://velog.io/@tsi0521/%EB%AF%B8%EB%A1%9C-%EC%B5%9C%EB%8B%A8-%EA%B2%BD%EB%A1%9C-DFS-%EC%99%80-BFS-%EB%B9%84%EA%B5%90)
...
![image](https://github.com/BH946/bh946.github.io/assets/80165014/e85fd36e-48b9-41d1-b944-fe80e49612ab) 

<br>

**p.dist 같이 좌표 말고도 기록하는 BFS**
...
* 근데, 아래 구조의 경우는 당연하게도 visited ("백트래킹")은 필요가 없다. 그저 브루트포스
* 물론, visited를 써서 아래처럼 접근도 가능하지만 이 경우엔 애초에 index 넘겨서 visited없이 바로하는게 훨씬 빠름
  * visited의 경우 구별은 되지만 하나하나 다 접근해서 방문여부 체크하는 시간이 있기 때문

<img src="https://github.com/BH946/spring-first-roadmap/assets/80165014/9a22e789-2635-412d-9e3a-fadeee2f39cc" alt="제목 없음" style="zoom:50%;" /> 
