...
- (추가) **부분집합(조합과 유사)** -> 문제참고 : [도영이가...](https://www.acmicpc.net/problem/2961)
  - **"있다 or 없다"** 로 경우의 수를 구하면 -> `부분집합`
  - power set(=멱집합) : 집합론에서 멱집합은 주어진 집합의 모든 부분 집합들로 구성된 집합
  - 문제 지문 중에 "적절히 섞어서" 이런 지문을 부분집합으로 해석 가능. WHY??
  - **<sub>N</sub>C<sub>1</sub>, <sub>N</sub>C<sub>2</sub>, <sub>N</sub>C<sub>3</sub>, ..., <sub>N</sub>C<sub>n</sub>** 이런식으로 탐색하라고 해석 되니까!
  - 만약 공집합도 포함이면?? <sub>N</sub>C<sub>0</sub> 까지 포함하는 것
  - **"암기"** -> visited의 T/F 에 해당하는 dfs 재귀 2개 사용
- (추가) **"완탐"**은 조합적 문제에 자주등장 -> "**순조부**(순열,조합,부분집합)"
  - **“순서”**가 의미 있는지, 뽑힌 애들의 각 **“자리”**가 의미 있는지 ⇒ `순열`
    - "중복순열, 중복조합은?" -> 중복이 존재하는것 (EX : 112)
    - 사진 코드 참고 : [1/31(수) 알고리즘 - 조합](https://www.notion.so/1-31-12b93b0e3acb4cb59587d4bff10d0642)
  - “**순서”**가 의미 없는지, 뽑힌 애들의 각 **“자리”**가 의미 없는지 ⇒ `조합`
    - 즉, “ABC” 와 “BAC” 가 (순서)의미있냐? 없으면 `조합`
    - 사진 참고 : [1/30(화) 알고리즘 - 완전탐색](https://www.notion.so/1-30-02242d871d45442a80d3b4dd896cd66f)
  - **"암기"**
    ...
    - 중복조합 → idx (조합과 한끗차이)
- (추가) 알고리즘 분석할 때 **"완탐"**을 생각할텐데 꼭 간단한 **for문으로 되는지부터 판단**하자!
  - "규칙"을 찾아 for문으로 간단히 해결,
  - 재귀할 분기가 딱히 없을때도 간단히 해결,
  - 순열 조차도 for문으로 표현 가능 -> {1,2,3} 정도 크기에 3중 for문으로 가능
    - (즉, nPr 에서 r이 2~3개 내외) 고정된 경우 → **반복문** 중첩구현 간단
    - 가변된 경우 → **재귀**
- 무조건 알고리즘 분석할 때 **"종이, 펜 활용"**
  - **"상태트리"** 를 그려보거나 조건에 맞게 **"예제를 풀이"**
  - 타입 범위는 애매하면 무조건 `long`
- **브루트포스(완전탐색)** 우선 생각 -> 복잡도만 적절하다면 코딩테스트에는 이게 최고라 생각!
  - **"완.탐"에 DFS(재귀+백트래킹)**이나 **"최단거리"면 BFS(+DP)** 로 풀어나가는 편인데,
  - 추가 조건없이 "for문이나 재귀"로 풀릴만한 복잡도면 **꼭 복잡하게 풀지말고 시도!!**
  - 백트래킹은 따로 BFS에서 구현 안해봤기에 DFS(재귀)로 꼭 구현하자
  - **(중요) "완.탐" 순,조,부 로 생각하면 됨!! 복잡도도 순조부 복잡도 고려**
    - 순열은 n의 수가 10보다 클 경우 순열 문제가 아닐 가능성이 높음
      - 순열의 시간복잡도 n!
    - 조합은 n=30, r=15일 경우 약 1억 5천 → <sub>30</sub>C<sub>15</sub> = 155117520
    - 부분 집합은 n이 30일 경우
      - 2^20은 약 100만, 2^30은 약 10억
- 복잡도?? 너무 크거나 나머지 상황은 DP, 그리디 등으로 접근
  - **DP 여러가지 유형** 미리 익숙해두자
- 아래 **자료구조** 정도는 기억해두고 필요한지 판단해서 사용하자
  - 스택&큐&덱, 이분탐색, 해시맵, 우선순위 큐 -> 사용법 기억
  - `binarySearch, lower_bound, upper_bound` **직접 구현 방법 기억**
  - 우선순위 큐 -> 백준 **나무 재테크, 이차원 배열...(17140)** 참고
    - **compareTo** 오버라이딩 직접 하는게 빠름 -> `implements Comparable<>`
    - !qu.iseEmpty() 보다 **size** 미리 구해서 사용이 빠름
    - 우선순위 큐 최대한 낭비없이 사용 권장
...
**자주 나오는 로직은 그냥 외우자 -> "시뮬레이션"**

* EX1) 2차원 **배열 회전 로직**은 `B[x][y]=A[N-1-y][x]`
  * 단 x,y가 0부터 시작 -> N-1, M-1은 끝자리 의미
  * 실제로 예시 좌표 그려보면서 구하면 공식을 금방 도출할 수 있다.
  * **추가1) 배열돌리기1** 같은 회전 로직은 `rotate(x, y, N, M)` 
    * 해당 함수로 회전로직을 구현해놓고 **여러 시작점(x,y), 끝점(N,M)** 을 줘서 구동!
  * **추가2) 2차원 "달팽이"**처럼 빙글빙글 돌때 **"범위체크+visited"**로 간단히 구현 가능하다.
    * **좌표 원복**하고 방향 전환하는 형식이 될 것이다.
* EX2) **투포인터**의 개념을 잘 생각 -> 두개의 배열을 인덱스 따로 하여 따로 동작하고 싶을때도 있기에 이 개념을 기억해두자
* EX3) **배열의 인덱스를 실제 숫자**로 치환하여 생각하는 방식 -> 숫자 빈도수 구하거나 등등...
* EX4) **dx, dy**로 1칸씩 같던 것을 **끝까지 이동(while)**하는것 -> dx, dy를 좌표에 더할수록 해당 방향으로 계속 이동 가능하단 점을 알아두자
  * **추가1) 2048 or 모노미노도미노2** 처럼 **끝까지 이동(while) + 범위 넘으면 원복(--)** 방식 괜찮음
  * 이것을 "하나의 함수" 로 구성하여 "각각의 좌표나 행,열"로 각각 재사용하는것이 편함
  * "오른쪽 방향일때 오른쪽부터 접근해야하는가" 이런것도 꼭 고려!

* EX5) 보통 "시뮬레이션"은 복잡도가 높지않아 **배열 상태 매번 새로 복제** 잘 고려 -> 한번씩 이런 사소한 할당에서 문제가 발생
  * `int[][] backup = new int[10][10];`
  * 이후에 backtracking 에 사용할 수도 있으니 이런 복제도 잘 고려
* EX6) 2차원 좌표 **구간합** 방식 -> `sum[i-1][j] ` 와 `가로방향(->) 누적값` 과 `inArr[i][j] 현재값`
  * 즉, `sum[i][j] = sum[i-1][j](바로위) + 해당 i행의 가로방향(->) 누적값 + inArr[i][j]`
* EX7) **반시계는 시계방향 3번**과 동일하다 -> **단, 90도일때!!** 90도 아니면 해당아님!!

- EX8) "정렬 암기" -> `Long.compare(a.x, b.x) 는 a.x < b.x ? -1 : (a.x==b.x?0:1) 와 동일`

  - (1)우선순위큐 -> 객체 만들때 **Comparable의 compareTo** 오버라이딩

    - `static class Pair implements Comparable<Pair> {}`

  - (2)Arrays, Collections -> **Comparator의 compare** 오버라이딩 방식도 존재 

    - `static class ObjectSort implements Comparator <Integer> {}`

    - ```java
      ObjectSort ob = new ObjectSort();
      Arrays.sort(inArr, ob); // ob 대신 "람다식"으로도 바로 가능
      Collections.sort(inArr, (o1,o2)->o1.itemId - o2.itemId); //"lambda"
      ```

- EX9) **"빈도수"** 구할때 List[] 도 있지만, **HashMap** 에 담으려 노력해보자 (자료구조 활용)

  - `Map<Integer, Integer> map = new HashMap<>(); // <number, count>`
  - `hm.put("java", hm.getOrDefault("java", 0)+1);` -> put, get 함수 기억

- EX10) **슬라이딩 윈도우 알고리즘 (Sliding Window)**

  - [DNA 비밀번호](https://www.acmicpc.net/problem/12891) -> 참고 문제

  - **맨 앞 문자는 제거, 새문자가 맨 뒤에 추가** -> 이 문장이 핵심 알고리즘 로직이다.
...
**아래에 정리한 BFS(최단거리.최단 경로 트리.0-1BFS)+DFS(재귀.백트래킹) 부분을 우선 확인**
...
- [최단 경로 트리](https://bh946.github.io/baekjoon/(%EB%B0%B1%EC%A4%80_%ED%95%84%EC%88%983)%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98-%ED%85%9C%ED%94%8C%EB%A6%BF-java/#%EC%B5%9C%EB%8B%A8-%EA%B2%BD%EB%A1%9C-%ED%8A%B8%EB%A6%AC)
  - **다익스트라(아래코드기억)** : **하나의 정점**에서 다른 모든 정점으로의 최단경로 -> **우선순위 큐 + 최소거리 배열**
  - 플로이드 : **모든 정점**에서 다른 모든 정점으로의 최단경로 -> **3중 for문 + 인접행렬**
  - 벨만포드 : 다익스트라는 양수만, 벨만포드는 음수도 최단경로 사용 가능
    - 근데, 벨만포드까지는 코테 잘 안나올것 같아서 나중에 시간나면 보는걸로..

- MST(최소신장트리)  
  **최단경로트리와 차이점은 "여러 정점들을 거쳐 돌아가도 되느냐 아니느냐의 차이"**  
  MST의 경우 바로 인접 정점들중 최소 가중치를 구함
  - **프림(아래코드기억)** -> 우선순위 큐
  - 크루스칼 -> Union find
...
- 다수의 중요 알고리즘들이 원소들의 그룹에서 최적의 부분 집합을 찾는 것이다.(예:냅색)
...
**bit mask -> visited를 대체하는것!**

- 참고 : 비트 시프트 연산의 출력값을 간단히 계산 : 2^k 로 생각
  - 2 << 3 이면 2*2^3
  - 2 >> 3 이면 2/2^3
- & : 특정 자리가 0인지 1인지 확인하고 싶을때 사용
- << : 자리(크기) 역할
- | : 특정 자리를 1로 변경
- EX : `(i & (1<<j)) != 0`
  - `1<<j` 는 인덱스 역할
  - i비트의 1<<j 번째를 & 연산으로 1인지 확인하는 것

![image](https://github.com/BH946/bh946.github.io/assets/80165014/a9d48972-f408-4a67-8d02-2422bb545a9a) 

<br>

**binary counting은 2진수로 표현 가능한 경우의 수 → for문으로**

- {1, 3, 5, 6, 9} 가 있을때 이것의 모든 부분집합을 아래의 2진수로 표현이 가능!!
...
- 물론 **본인은 완.탐 부분에는 DFS(재귀), 최단거리는 BFS** 를 사용하고 있다.  
  그러기 위해선 DFS, BFS를 정말 정확하게 이해하고 있어야 한다.
...
- **방문기록은 방문 안한곳만 큐에 넣어 탐색하기 위한 목적**
  ...
  - 이 4방향이 트리의 가지로 자꾸 착각하게 되는데, 가지는 "좌표"로 생각할 것
- 일반적인 트리와 코드 모양??
  - 큐SIze는 "가지수" -> 총 개수로 볼 수 있다.
  - "레벨" -> 최단거리로 볼 수 있다.

![image](https://github.com/BH946/bh946.github.io/assets/80165014/5a99ed1f-c915-4c03-9310-7c4cf2cda987)

```java
// 동일 depth에서 각 BFS 결과들 확인 위해서는 for(size만큼) 끝난 아래 자리를 확인!!
// (큐가 비엇을때도 놓치지 않기 위해서)
```
...
* **자주 나오는 로직은 그냥 외우자**
  ...
  * EX2) **투포인터**의 개념을 잘 생각 -> 두개의 배열을 인덱스 따로 하여 따로 동작하고 싶을때도 있기에 이 개념을 기억해두자
  * EX3) **배열의 인덱스를 실제 숫자**로 치환하여 생각하는 방식 -> 숫자 빈도수 구하거나 등등...
  * EX4) **dx, dy**로 1칸씩 같던 것을 **끝까지 이동**하는것 -> dx, dy를 좌표에 더할수록 해당 방향으로 계속 이동 가능하단 점을 알아두자
  * EX5) 보통 "시뮬레이션"은 복잡도가 높지않아 **배열 상태 매번 새로 복제** 잘 고려 -> 한번씩 이런 사소한 할당에서 문제가 발생
    * `int[][] backup = new int[10][10];`
    * 이후에 backtracking 에 사용할 수도 있으니 이런 복제도 잘 고려
...
public static void main(String[] args) {
    ObjectSort ob = new ObjectSort();
    Arrays.sort(inArr, ob); // ob 대신 람다식으로도 바로 가능
...
**이진트리로 순회했을때 값이 필요할 경우에 사용 (즉, 순회한 값을 모를때)**
...
* **전위, 중위, 후위 순회**
  * 전위 : 위 -> 왼 -> 오
  * 중위 : 왼 -> 위 -> 오
    * 특히 왼쪽에서 오른쪽으로 차례대로 순서를 정한것과 같다.
  * 후위 : 왼 -> 오 -> 위
* **순회는 구현 쉬워서 코드 생략**
...
public Boolean isBinaryTree(String binary) {
    ...
    int root = binary.length()/2; // 이진트리의 루트노드 찾는 간단한 방식
...
**사칙연산 유효성 검사_1233 SWEA**

- 입력은 1번 노드부터 N번 노드까지 이진트리 형태로 주어짐

```java
// 반복문 ver -> 꼭 재귀가 아니어도 이렇게 풀이가 된다는것
static int N, ans;
static char[] node;
static StringBuilder sb = new StringBuilder();
