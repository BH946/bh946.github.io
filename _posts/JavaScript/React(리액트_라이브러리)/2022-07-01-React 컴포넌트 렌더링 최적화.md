...
categories : React_JS
tag : [JavaScript, 리액트, React, 생명주기, 컴포넌트, 함수형 컴포넌트, 클래스형 컴포넌트, useEffect, useState, setState, 반복 렌더링, useMemo, useCallback, memo, 마운트와 언마운트, 재렌더(업데이트), Material UI, 재렌더와 리페인트 차이, 반복 렌더링, ..., Spread, const {}, const [], Destructuring]
toc: true
toc_sticky: true
...
* 컴포넌트에서 **값(상태)의 변화를 감지**함
  ...
  * 컴포넌트에서 관리하는 값(props, state)의 변화를 mount
...
* 실행되면서 함수 내에 정의된 함수들(**onClick 리스너 등) 혹은 변수**들은 **새로 선언**됨
